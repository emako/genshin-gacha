...
def main():
    ...
    if s.getKey("FLAG_SHOW_REPORT"):
        subprocess.Popen("genshin-gacha-analyzer.exe")
...
if __name__ == "__main__":
    ...
    latest = "https://pd.zwc365.com/seturl/https://raw.githubusercontent.com/emako/genshin-gacha/main/src/exporter/VERSION.md"
    try:
        print("检查更新中...", end="", flush=True)
        latestVerison = requests.get(latest).text
        verison = s.getKey("verison")
        if verison != latestVerison:
            print(f"当前版本{verison}不是最新\n请到 https://github.com/emako/genshin-gacha/releases 下载最新版本{latestVerison}")
    except Exception:
